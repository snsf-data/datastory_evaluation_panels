...
datastory_name <- "evaluation_panels"
...
# characters etc.)
datastory_name_de <- "zentrale-rolle-der-evaluationsgremien"
datastory_name_en <- "key-role-of-evaluation-panels"
datastory_name_fr <- "panels-au-coeur-des-procedures-de-selection"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Key role in evaluation procedure: the evaluation panels and their members" 
lead_en <- "The SNSF's National Research Council decides whether or not to fund applications. The 89 evaluation panels handle the preparatory work on which it bases its decisions, assessing several thousand applications each year." 
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Zentrale Rolle in den Auswahlverfahren: Die Evaluationsgremien und ihre Mitglieder" 
lead_de <- "Der Nationale Forschungsrat des SNF entscheidet darüber, ob ein Gesuch gefördert wird oder nicht. Die 89 Evaluationsgremien bereiten seine Entscheide vor. Jährlich beurteilen sie mehrere Tausend Gesuche." 
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Les panels d’évaluation et leurs membres sont au cœur des procédures de sélection"
lead_fr <- "Le Conseil national de la recherche du FNS décide si une requête est financée ou non. Les 89 panels d’évaluation préparent ses décisions. Chaque année, ils évaluent plusieurs milliers de requêtes." 
...
publication_date <- "2021-12-21 01:00:00"
...
doi <- "10.46446/datastory.central-role-of-evaluation-panels" 
# URL to Github page (optional)
github_url <- "https://github.com/snsf-data/datastory_evaluation_panels" 
# Put Tag IDs here. Only choose already existing tags.
tags_ids <- c(280, # Panels
              250, # Peer-review
              60) # Gender equality
